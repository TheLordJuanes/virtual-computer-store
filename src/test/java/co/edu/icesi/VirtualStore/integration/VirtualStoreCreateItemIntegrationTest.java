...
@SpringBootTest
public class VirtualStoreCreateItemIntegrationTest {
    ...
    @Test
    @SneakyThrows
    public void testCreateItemSuccessfully() {
        ItemDTO baseItem = baseItem();
        String body = objectMapper.writeValueAsString(baseItem);
        MvcResult result = mockMvc.perform(MockMvcRequestBuilders.post("/createItem").contentType(MediaType.APPLICATION_JSON).content(body)).andExpect(status().isOk()).andReturn();
